...
public class FriendDaoImpl implements FriendDao{
	...
	public List<User>getAllSuggestedUsers(String username)
	{
		...
		SQLQuery query=session.createSQLQuery("select*from user_batch4 where username in(select username from user_batch4 where username!=? minus(select fromId from friend_batch4"
				+ "where toId=? union select toId from friend_batch4 where fromid=?))");
